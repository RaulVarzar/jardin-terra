    ...
    <motion.div className="grid relative bg-secondary h-full shadow-lg group max-lg:grid-rows-7 lg:grid-cols-7 z-50  w-[90vw] xl:w-[85vw] 3xl:w-[80vw]  max-w-screen-xl pb-8 p-3 md:p-4 lg:p-4  rounded-2xl lg:rounded-3xl ">
      ...
      <div className="z-0 touch-pan-y flex flex-col justify-start lg:justify-center gap-6 px-1 py-6 mx-auto overflow-auto lg:gap-8 sm:py-8 md:px-5 xl:px-12 max-sm:row-span-4 sm:max-lg:row-span-3 lg:col-span-4 xl:col-span-3">
        <div className="flex flex-col md:gap-1 xl:gap-2">
          <motion.h1 layoutId={title} className="p-0 pb-2 sm:pb-4  my-0">
            <SplitLinesAnimation
              text={title}
              duration={1}
              stagger={0.1}
              once={false}
              className="max-md:text-pretty md:text-balance uppercase text-2xl font-bold leading-none md:text-3xl  lg:text-4xl  2xl:text-5xl text-neutral-content"
            />
          </motion.h1>

          <SplitLinesAnimation
            text={descriptions[0].content}
            layoutId={descriptions[0].content}
            duration={1}
            delay={0.2}
            stagger={0.1}
            className="text-sm brightness-75 max-sm:line-clamp-6 font-light leading-tight md:leading-snug md:tracking-wider sm:text-base text-balance md:text-md xl:text-lg text-neutral-content"
          />
        </div>
      </div>
    </motion.div>
