...
const Card = ({ item, setSelectedId }) => {
  ...
  return (
    <motion.div
      ...
      layout="position"
      className="grid relative bg-secondary group shadow-sm transition duration-300 group max-lg:grid-rows-7 lg:grid-cols-7 w-[95vw]  max-h-[960px] place-self-center h-full xl:w-[85vw] 3xl:w-[80vw] max-w-screen-2xl p-2 md:p-4 xl:p-6 negative-shado cursor-pointer rounded-2xl  "
    >
      ...
      <div className="z-20 flex flex-col justify-center gap-6 px-3 py-8 mx-auto overflow-auto lg:gap-8 md:px-5 xl:px-12 max-sm:row-span-4 sm:max-lg:row-span-3 lg:col-span-4 ">
        ...
        <span className="text-2xl md:text-3xl z-50 absolute top-4  right-4 p-3 transition duration-300 md:p-4   text-base-content brightness-85 group-hover:brightness-125 group-hover:scale-105">
          <MdOpenInFull />
        </span>
      </div>
    </motion.div>
